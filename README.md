...
### **[▶ 立即在线试玩](https://whouare-dot.github.io/xiantu/)** —— 无需安装，打开即玩
...
▶ **[观看演示视频](https://www.bilibili.com/video/BV1zQYB6oES8)** · 3 分 30 秒 · B站 ｜ [站内播放](https://whouare-dot.github.io/xiantu/video.html)
...
需要 Python 3.8+（仅用于起静态服务器，游戏本身不依赖 Python）。端口被占用时会自动往后找可用端口。
...
**数值由目标挂机时长反推生成** 
26 个境界的需求修为由 `tools/balance_sim.py` 反推：
先确定好每个境界的**目标挂机时长**，再按 `needCult = baseSpeed × 目标时长` 生成，
...
**价格由计算公式反推得来。** 丹药与装备的坊市价一律锚定**材料成本**：
...
下界保证了不会出现「买成品比买原料还便宜」的倒挂；上界预防了利用差价刷灵石。
`tools/test_economy.mjs` 里有对应的双向断言。详见 `docs/设计文档.md` 的 2.5.1。
...
**洞府离线也推进。** 建筑升级用绝对时间戳计时，关闭后台游戏照常运行；
...
v1.0.0 是完整的一周目：从炼气到飞升，含境界突破与天劫、回合制战斗、功法参悟、
炼丹炼器、洞府离线产出、宗门与立场、灵兽血脉、道侣、坊市与拍卖行、奇遇与探险、
试炼塔、轮回转生与真结局、世界天象、成就图鉴、本世功课，以及从 demo 1.x 无损迁移的存档。

逐项玩法规则见 [`docs/玩法总览.md`](docs/玩法总览.md)。
...
`src/music/` 在本仓库中是**空的**。开发时存放的音频无法随开源仓库公开分发，因此已从版本控制中排除，但作为推荐列表展示在src\music\README.md。
